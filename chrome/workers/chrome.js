...
app.window = {
  ...
  get id() {
    return app.storage.read("window.id") !== undefined
      ? app.storage.read("window.id")
      : "";
  },
  create: function (options, callback) {
...
app.interface = {
  ...
  get id() {
    return app.storage.read("interface.id") !== undefined
      ? app.storage.read("interface.id")
      : "";
  },
  ...
  create: function (url, callback) {
    app.window.query.current(function (win) {
      ...
      const top =
        config.interface.size.top ||
        win.top + Math.round((win.height - height) / 2);
      const left =
        config.interface.size.left ||
        win.left + Math.round((win.width - width) / 2);
      app.window.create(
        {
          url: url,
          top: top,
