var config = {
  ...
  menu: {
    custom: {
      title: "File",
      items:
        "newdocument openfile separator save saveas reset separator print preview darkmode",
    },
  },
  toolbar: `
    sizeselect |
  ...
  load: function () {
    config.storage.load(function () {
      const theme = window.matchMedia("(prefers-color-scheme: dark)").matches
        ? "dark"
        : "default";
      const skin = window.matchMedia("(prefers-color-scheme: dark)").matches
        ? "oxide-dark"
        : "oxide";
      config.interface.skin =
        config.storage.read("skin") !== undefined
          ? config.storage.read("skin")
          : skin;
      config.interface.theme =
        config.storage.read("theme") !== undefined
          ? config.storage.read("theme")
          : theme;
      config.download.id =
        config.storage.read("id") !== undefined
          ? config.storage.read("id")
          : "";
      config.download.content =
        config.storage.read("content") !== undefined
          ? config.storage.read("content")
          : "";
      config.download.type =
        config.storage.read("type") !== undefined
          ? config.storage.read("type")
          : "text/html";
      config.download.filename =
        config.storage.read("filename") !== undefined
          ? config.storage.read("filename")
          : "result.html";
      config.app.start();
    });
  ...
  app: {
    start: function () {
      ...
      dark.addEventListener(
        ...
        function () {
          config.interface.theme =
            config.interface.theme === "default" ? "dark" : "default";
          config.interface.skin =
            config.interface.skin === "oxide" ? "oxide-dark" : "oxide";
  ...
  interface: {
    ...
    on: {
      ...
      init: function (e) {
        ...
        config.editor.ui.registry.addIcon(
          "reset",
          `
  ...
  listener: {
    ...
    reset: function () {
      const flag = window.confirm(
        "Are you sure you want to reset the editor content (to the last saved point)?",
      );
      if (flag) {
    ...
    fileio: {
      url: function () {
        config.download.content = tinyMCE.activeEditor.getContent({
          format: "raw",
        });
        config.download.type = config.download.type
          ? config.download.type
          : "text/html";
        return URL.createObjectURL(
          new Blob([config.download.content], { type: config.download.type }),
        );
      },
